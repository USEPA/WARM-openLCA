...
import gov.epa.warm.rcp.utils.OS;
...
public class BrowserFactory {
	...
	private static void initialize() {
		...
		if (OS.getCurrent() != OS.Windows || "x86".equals(System.getProperty("os.arch"))) {
	...
	private static void initializeXulRunner(String xulRunnerPath) {
		try {
			log.trace("Use Mozilla browser, XUL runner found at {}", xulRunnerPath);
	...
	private static Browser createMozilla(Composite parent) {
		try {
			...
			log.warn("Failed to create Mozilla browser, fall back to system default", e);
			useXulRunner = false;
	...
	private static void initMozillaPrefs() {
		...
		try {
			...
			InputStream is = BrowserFactory.class.getResourceAsStream("user.js");
